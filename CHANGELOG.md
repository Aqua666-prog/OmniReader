...
- Поиск по большим документам получил streaming next/previous, wrap-around, пропуск отдельных повреждённых страниц и защиту от повторного возврата того же единственного результата.
- Исправлены race condition индикатора выделения/OCR, устаревшие результаты задач, очередь повторяющихся ошибок тайлов и лимит Clipboard для межстраничного текста.
- Многостраничные заметки/цитаты сохраняют геометрию в совместимом v2 JSON без новой Room-миграции.
- Добавлены Unicode/RTL, OCR, отмена, 2001-page search и regression tests.
- Version code/name: 8 / 0.8.0. APK по запросу пользователя в этой сессии не собирался.
